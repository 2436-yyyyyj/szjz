...
- 加载页封面改为仓库已存在的 WebP 首屏渲染，并添加预加载，避免依赖未上传的 JPG。
- 加载页已移除倒计时浮层，保留点击或自动进入逻辑。
- 手机首页图片使用 object-fit: contain，完整显示，不再裁切。
- 手机端布局改为先显示公司名，再显示产品分类，再显示具体产品列表。
- 产品列表一次渲染全部 151 个型号；前 24 张优先加载，后续图片滚动到视口时加载。
- 手机端点击任意型号会立即加载详情图，并自动跳到详情区。
...
https://2436-yyyyyj.github.io/szjz/?v=20260619-company-first
