...
function renderProducts() {
  ...
  grid.innerHTML = shown.map((product, index) => `
    <button class="product-card${product.code === selectedCode ? " is-selected" : ""}" type="button" data-product="${product.code}">
      <img class="product-image" src="${product.image}" data-fallback-src="${product.fallback}" alt="${product.code} ${product.categoryName}" loading="eager" decoding="async" fetchpriority="${index < 8 ? "high" : "auto"}">
...
function registerCacheCleaner() {
  ...
  navigator.serviceWorker.register("./sw.js?v=20260619-no-countdown").catch(() => {});
}
