...
function renderProducts() {
  ...
  grid.innerHTML = shown.map((product, index) => `
    <button class="product-card${product.code === selectedCode ? " is-selected" : ""}" type="button" data-product="${product.code}">
      <img class="product-image" src="${product.image}" alt="${product.code} ${product.categoryName}" loading="eager" decoding="async" fetchpriority="${index < 8 ? "high" : "auto"}">
  ...
  if (image) {
    image.addEventListener("error", () => {
      if (!image.dataset.triedFallback) {
        ...
        image.src = "./assets/brand/splash.webp";
      }
    }, { once: true });
  }
...
function registerCacheCleaner() {
  ...
  navigator.serviceWorker.register("./sw.js?v=20260618-final").catch(() => {});
}
